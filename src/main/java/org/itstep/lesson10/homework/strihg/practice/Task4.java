package org.itstep.lesson10.homework.strihg.practice;
...
public class Task4 {
    ...
    public static void main(String[] args) {
        String vowels = "eioayu";//голосні
        String lorem = "It is a long established fact that " +
        ...
        for (int i = 0; i < words.length; i++) {
            if (!words[i].isBlank() && words[i].length() > 1) {
                if (vowels.contains(String.valueOf(words[i].charAt(0))) && !vowels.contains(String.valueOf(words[i].charAt(words[i].length()-1)))){
